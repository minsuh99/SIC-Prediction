...
train_dataset = ClimateSICDataset(climate_array=climate_data, sic_array=sic_data, mask_array=mask, window_length=12, prediction_length=6, start_idx=0, end_idx=287)
val_dataset = ClimateSICDataset(climate_array=climate_data, sic_array=sic_data, mask_array=mask, window_length=12, prediction_length=6, start_idx=288, end_idx=323)
test_dataset = ClimateSICDataset(climate_array=climate_data, sic_array=sic_data, mask_array=mask, window_length=12, prediction_length=6, start_idx=324, end_idx=359)
...
class SeaIceLSTM(nn.Module):
    def __init__(self, input_channels=11, hidden_size=64, height=428, width=300, pred_L=6):
...
num_epochs = 30
model = SeaIceLSTM(input_channels=11, hidden_size=64, pred_L=6).to(device)
...
# Visualization
def plot_sic_error_map(pred, true, mask, x_coords, y_coords, dates, start_idx, save_dir='./results/LSTM_6_Climate_SIC'):
