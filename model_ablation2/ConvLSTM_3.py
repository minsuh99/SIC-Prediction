...
train_dataset = ClimateSICDataset(climate_array=climate_data, sic_array=sic_data, mask_array=mask, window_length=12, prediction_length=3, start_idx=0, end_idx=239)
val_dataset = ClimateSICDataset(climate_array=climate_data, sic_array=sic_data, mask_array=mask, window_length=12, prediction_length=3, start_idx=240, end_idx=299)
test_dataset = ClimateSICDataset(climate_array=climate_data, sic_array=sic_data, mask_array=mask, window_length=12, prediction_length=3, start_idx=300, end_idx=359)
...
class SeaIceConvLSTM(nn.Module):
    def __init__(self, input_channels=11, hidden_channels=64, kernel_size=(3, 3), lstm_layers=1, pred_L=3, bias=True):
...
num_epochs = 30
model = SeaIceConvLSTM(input_channels=11, hidden_channels=64, kernel_size=(3,3), lstm_layers=1, pred_L=3, bias=True).to(device)
...
for epoch in tqdm(range(1, num_epochs+1), desc="Training Progress", leave=True):
    ...
    if avg_val_loss < best_val_loss:
        ...
        }, 'best_seaice_convlstm_3_Climate_SIC.pth')
...
checkpoint = torch.load('best_seaice_convlstm_3_Climate_SIC.pth', map_location=device)
...
# Visualization
def plot_sic_error_map(pred, true, mask, x_coords, y_coords, dates, start_idx, save_dir='./results/ConvLSTM_3_Climate_SIC'):
