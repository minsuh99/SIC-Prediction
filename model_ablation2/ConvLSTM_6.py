...
class SeaIceConvLSTM(nn.Module):
    def __init__(self, input_channels=11, hidden_channels=64, kernel_size=(3, 3), lstm_layers=1, pred_L=6, bias=True):
...
num_epochs = 30
model = SeaIceConvLSTM(input_channels=11, hidden_channels=64, kernel_size=(3,3), lstm_layers=1, pred_L=6, bias=True).to(device)
...
for epoch in tqdm(range(1, num_epochs+1), desc="Training Progress", leave=True):
    ...
    if avg_val_loss < best_val_loss:
        ...
        }, 'best_seaice_convlstm_6_Climate_SIC.pth')
...
checkpoint = torch.load('best_seaice_convlstm_6_Climate_SIC.pth', map_location=device)
...
# Visualization
def plot_sic_error_map(pred, true, mask, x_coords, y_coords, dates, start_idx, save_dir='./results/ConvLSTM_6_Climate_SIC'):
