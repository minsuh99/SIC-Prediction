...
class SeaIceTransformer(nn.Module):
    def __init__(self, input_channels=11, height=428, width=300, d_model=512, nhead=8, num_layers=4, pred_L=1):
        super().__init__()
        self.height = height
        self.width = width
        self.pred_L = pred_L
...
num_epochs = 30
model = SeaIceTransformer(input_channels=11, hidden_size=64, pred_L=3).to(device)
...
for epoch in tqdm(range(1, num_epochs+1), desc="Training Progress"):
    ...
    if avg_val_loss < best_val_loss:
        ...
        }, 'best_seaice_Transformer_3_Climate_SIC.pth')

    print(f"[Epoch {epoch}/{num_epochs}] Train Loss = {avg_train_loss:.6f}  |  Val Loss = {avg_val_loss:.6f}  |  LR = {optimizer.param_groups[0]['lr']:.2e}")

## Test & Visualization
checkpoint = torch.load('best_seaice_Transformer_3_Climate_SIC.pth', map_location=device)
...
# Visualization
def plot_sic_error_map(pred, true, mask, x_coords, y_coords, dates, start_idx, save_dir='./results/Transformer_3_Climate_SIC'):
