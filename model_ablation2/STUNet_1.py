...
class SeaIceSTSTUNet(nn.Module):
    def __init__(self, input_channels=11, tcn_channels=64, tcn_layers=3, STunet_features=[64, 128, 256, 512], pred_L=1):
        super().__init__()
        self.pred_L = pred_L
...
num_epochs = 30
model = SeaIceSTSTUNet(input_channels=11, tcn_channels=64, tcn_layers=3, STunet_features=[64,128,256,512], pred_L=1).to(device)
...
for epoch in tqdm(range(1, num_epochs+1), desc="Training Progress", leave=True):
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), 'best_seaice_STUNet_1_Climate_SIC.pth')
...
## Test & Visualization
model.load_state_dict(torch.load('best_seaice_STUNet_1_Climate_SIC.pth', map_location=device))
model.eval()
...
def plot_sic_error_map(pred, true, mask, x_coords, y_coords, dates, start_idx, save_dir='./results/STUNet_1_Climate_SIC'):
