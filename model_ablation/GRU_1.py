...
num_epochs = 30
model = SeaIceGRU(input_channels=1, hidden_size=64, pred_L=1).to(device)
...
for epoch in tqdm(range(1, num_epochs+1), desc="Training Progress"):
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), 'best_seaice_GRU_1_SICOnly.pth')

    print(f"[Epoch {epoch}/{num_epochs}] Train Loss = {avg_train_loss:.6f}  |  Val Loss = {avg_val_loss:.6f}  |  LR = {optimizer.param_groups[0]['lr']:.2e}")

## Test & Visualization
model.load_state_dict(torch.load('best_seaice_GRU_1_SICOnly.pth', map_location=device))
model.eval()
...
def plot_sic_error_map(pred, true, mask, x_coords, y_coords, dates, start_idx, save_dir='./results/GRU_1_SICOnly'):
